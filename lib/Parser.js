...
function Parser(states, options) {
	...
	if(options) {
		...
		if(options.multiline || options.m) {
			flags += "m";
		}
		if(options.dotAll || options.s) {
			flags += "s";
		}
		if(options.unicode || options.u) {
			flags += "u";
		}
	}
	this.states = this.compileStates(states, flags);
}

Parser.prototype.compileStates = function(states, flags) {
	var result = {};
	Object.keys(states).forEach(function(name) {
		result[name] = this.compileState(states[name], states, flags);
	}, this);
	return result;
};

Parser.prototype.compileState = function(state, states, flags) {
	var regExps = [];
